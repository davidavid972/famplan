  ...
  const renderSectionContent = () => {
    switch (activeSection) {
      ...
      case 'sync':
        return (
          <div className="space-y-6">
            <div ref={authSectionRef} tabIndex={-1}>
              <h3 className="text-sm font-medium text-foreground mb-3">{t('auth_profile')}</h3>
              <div className="p-4 sm:p-5 bg-muted rounded-2xl border border-border space-y-3">
                {isConnected ? (
                  <>
                    <input
                      type="email"
                      value={email}
                      readOnly
                      className="w-full px-4 py-3 rounded-xl border border-border text-sm bg-card text-foreground"
                      dir="ltr"
                    />
                    <button
                      onClick={disconnect}
                      className="w-full sm:w-auto min-h-[44px] px-6 py-3 text-sm text-destructive hover:bg-destructive/10 rounded-xl font-medium transition-colors border border-destructive/30"
                    >
                      {t('auth_disconnect')}
                    </button>
                  </>
                  ...
                  <>
                    <button
                      ...
                      className="w-full min-h-[44px] flex items-center justify-center gap-2 px-6 py-4 bg-card border-2 border-border rounded-xl hover:bg-muted hover:border-border transition-colors font-medium text-foreground disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                    ...
                    {connectError && <p className="text-sm text-destructive">{t('auth_connect_error')}</p>}
                  </>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <button type="button" onClick={() => setCalendarModalOpen(true)} className="flex flex-col items-center p-6 theme-surface min-h-[120px] w-full text-left cursor-pointer hover:shadow-md transition-all">
                <CalendarIcon className="w-8 h-8 text-primary mb-3" />
                <h3 className="font-medium text-foreground">{t('settings_cards_calendar_title')}</h3>
                <p className="text-sm text-muted-foreground text-center mt-1">{t('settings_cards_calendar_subtitle')}</p>
                <p className={`mt-2 text-xs font-medium ${isConnected ? 'text-primary' : 'text-accent'}`}>{isConnected ? t('auth_connected') : t('auth_not_connected')}</p>
              </button>
              <button type="button" onClick={handleDriveCardClick} className="flex flex-col items-center p-6 theme-surface min-h-[120px] w-full text-left cursor-pointer hover:shadow-md transition-all">
                <HardDrive className="w-8 h-8 text-primary mb-3" />
                <h3 className="font-medium text-foreground">{t('settings_cards_drive_title')}</h3>
                <p className="text-sm text-muted-foreground text-center mt-1">{t('settings_cards_drive_subtitle')}</p>
                <p className={`mt-2 text-xs font-medium ${isConnected ? 'text-primary' : 'text-accent'}`}>{isConnected ? t('auth_connected') : t('auth_not_connected')}</p>
              </button>
            </div>
